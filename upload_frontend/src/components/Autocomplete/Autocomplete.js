...
export const Autocomplete = forwardRef(
  (
    ...
    const clearButtonIsVisible =
      !props.disableClearable && !!(multiple ? value?.length : value);
    // React Hook Form
    ...
    const valueAsInputValue =
      transformValueToInputValue(valueProp) ||
      transformValueToInputValue(value);

    const handleChange = (event, selectedOption) => {
      if (isCreatable && selectedOption?.isCreatableOption) {
        setInputValue("");

        onCreate(selectedOption.inputValue)
          .then((option) => {
            if (!multiple) {
    ...
    const handleSearch = useCallback(
      debounce(({ search, loadedOptions, additionalData }) => {
